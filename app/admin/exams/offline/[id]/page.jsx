...
export default function EditOfflineExamPage({ params }) {
    ...
    const fetchDropdownsAndExam = async () => {
        try {
            setLoading(true);
            const [cRes, bRes, sRes, sessRes, scaleRes, examRes] = await Promise.all([
                fetch("/api/v1/courses"),
                fetch("/api/v1/batches"),
                fetch("/api/v1/subjects"),
                fetch("/api/v1/sessions"),
                fetch("/api/v1/grading-scales"),
                fetch(`/api/v1/offline-exams/${examId}`)
            ]);
            ...
            setFormData({
                ...
                course: examData.course?._id || examData.course || "",
                session: examData.session?._id || examData.session || "",
                batches: examData.batches ? examData.batches.map(b => b._id || b) : [],
                gradingScale: examData.gradingScale?._id || examData.gradingScale || "",
                weightagePercent: examData.weightagePercent ?? 100,
                isRankEnabled: examData.isRankEnabled ?? true,
                status: examData.status || "draft",
                entryDeadline: examData.entryDeadline ? new Date(examData.entryDeadline).toISOString().split('T')[0] : "",
                subjects: examData.subjects ? examData.subjects.map(s => ({
                ...
                coScholastic: examData.coScholastic ? examData.coScholastic.map(c => ({
                    paramName: c.paramName || "",
                    ratingScale: c.ratingScale || "A-E"
                })) : []
            });

        } catch (error) {
            toast.error(error.message || "Failed to fetch dependencies");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (!payload.gradingScale) payload.gradingScale = null;
            if (!payload.entryDeadline) payload.entryDeadline = null;
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-black text-slate-900 tracking-tight">Edit Offline Exam</h1>
                    <p className="text-slate-500">Modify the exam structure, max marks, and grading scale.</p>
                </div>
            </div>
            ...
            <Card className="p-6 space-y-8">
                ...
                <div className="pt-6 border-t border-slate-100 flex justify-end">
                    <Button size="lg" onClick={handleSubmit} disabled={saving} className="shadow-lg shadow-premium-blue/25">
