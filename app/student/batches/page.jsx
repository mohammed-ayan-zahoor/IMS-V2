...
export default function StudentBatchesPage() {
    ...
    const getStatus = (batch) => {
        // Simple logic: If start date is future -> Upcoming
        // If end date is past -> Completed
        // Else -> Active
        const now = new Date();
        ...
        if (start && start > now) return { label: "Upcoming", className: "bg-amber-100 text-amber-700" };
        if (end && end < now) return { label: "Completed", className: "bg-slate-100 text-slate-500" };
                                    ...
                                    <div className="flex items-start gap-3 text-sm text-slate-600">
                                        ...
                                        <div>
                                            {batch.schedule?.daysOfWeek?.length > 0 ? (
                                                batch.schedule.daysOfWeek.map(d => ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][d]).join(', ')
                                            ) : "Days TBD"}
