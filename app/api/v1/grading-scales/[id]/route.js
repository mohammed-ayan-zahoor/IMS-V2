...
export async function GET(req, { params }) {
    try {
        ...
        const scale = await GradingScale.findOne(query);
        if (!scale) {
        ...
        console.error("API Error [GradingScale GET]:", error);
...
export async function PATCH(req, { params }) {
    try {
        ...
        if (!scale) {
            return NextResponse.json({ error: "Grading scale not found or access denied" }, { status: 404 });
        }

        return NextResponse.json(scale);
    } catch (error) {
        console.error("API Error [GradingScale PATCH]:", error);
        return NextResponse.json({ error: error.message || "Failed to update grading scale" }, { status: 400 });
    }
}
...
export async function DELETE(req, { params }) {
    try {
        ...
        console.error("API Error [GradingScale DELETE]:", error);
        return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
    }
}
