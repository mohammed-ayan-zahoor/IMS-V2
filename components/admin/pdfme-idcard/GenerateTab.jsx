...
export default function GenerateTab() {
    ...
    // Roster refresh on filter changes
    useEffect(() => {
    ...
    const fetchTemplates = async () => {
        try {
            const tRes = await fetch("/api/v1/id-card-templates-pdfme");
            if (tRes.ok) {
    ...
    const downloadPDF = async (url, filename) => {
        try {
            ...
            a.href = blobUrl;
            a.download = filename || "id_card.pdf";
            ...
            console.error("PDF forced download failed", error);
    ...
    const handleGenerate = async () => {
        ...
        setGenerating(true);
        try {
            const res = await fetch("/api/v1/id-cards-pdfme/generate", {
                method: "POST",
            ...
            if (!res.ok) {
                ...
                throw new Error(errData.error || "ID Card generation failed");
            }

            const data = await res.json();
            
            if (data.success && data.studentCards) {
                toast.success(`Successfully generated ${data.studentCards.length} vector ID card(s)`);
                
                // Trigger browser batch downloads of the generated PDF files
                for (let i = 0; i < data.studentCards.length; i++) {
                    ...
                    setTimeout(() => {
                        ...
                        downloadPDF(card.pdfUrl, `ID_Card_${safeName}_${card.studentId || i}.pdf`);
                    }, i * 350); // Small delay to avoid browser blocking multiple downloads
                }
            } else {
                ...
                <div className="bg-gradient-to-br from-white to-blue-50/30 rounded-3xl border border-slate-200/60 shadow-sm p-6 space-y-6 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                        ...
                        <div>
                            ...
                            <p className="text-[11px] text-slate-500 uppercase tracking-widest font-black">PDF Generation</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-600 ml-1">Layout Template</label>
                            <select
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-600 ml-1">Roster Lifecycle</label>
                            <select
                                value={filter}
                                onChange={(e) => {
                        ...
                        {/* Standard/Course Filter */}
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-600 ml-1">
                                {instituteType === 'SCHOOL' ? 'Standard (Std)' : 'Course'}
                            </label>
                            <select
                                value={courseId}
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-600 ml-1">
                                {instituteType === 'SCHOOL' ? 'Section' : 'Batch'}
                            </label>
                            <select
                                value={batchId}
                    ...
                    <div className="pt-2">
                        ...
                        >
                            {generating ? (
                                <><Loader2 size={18} className="animate-spin mr-2" /> Generating PDFs...</>
                            ) : (
                                <>
                                    <Download size={18} className="mr-2" /> 
                                    Generate {selectedStudents.size > 0 ? selectedStudents.size : ""} Cards (PDF)
                                </>
                            )}
                        </Button>
                        <p className="text-[11px] text-slate-400 text-center mt-3 font-medium leading-relaxed px-2">
                            Vector-scale PDF files will be downloaded automatically.
                        </p>
                    </div>
                </div>
            </div>

            {/* 2. Main Students List */}
            <div className="lg:col-span-8 xl:col-span-9">
                <Card className="p-0 border-slate-200/60 shadow-sm rounded-3xl overflow-hidden bg-white flex flex-col h-[700px]">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            ...
                            <div className="relative w-full sm:w-72">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-medium placeholder:text-slate-400"
                                />
                                {loadingStudents && (
                        ...
                        <div className="flex items-center justify-between px-3 py-2.5 bg-slate-50/80 rounded-xl border border-slate-100">
                             <div className="flex items-center gap-2.5">
                                 <button
                                     ...
                                     className={cn(
                                         "w-5 h-5 rounded flex items-center justify-center transition-colors border outline-none",
                                        ...
                                        >
                                            ...
                                            <div className={cn(
                                                ...
                                                isSelected 
                                                    ...
                                                    : "bg-slate-50/80 border border-slate-300"
                                            )}>
