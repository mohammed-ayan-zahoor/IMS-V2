...
export async function cleanupOrphanedFiles() {
    try {
        ...
        const [certTemplates, idCardTemplates, materials, users, institutes, websitePages, websiteConfigs] = await Promise.all([
            ...
            WebsitePage.find({}, { draftContent: 1, liveContent: 1, sections: 1 }).lean(),
            WebsiteConfig.find({}, { "branding.logo": 1, "branding.favicon": 1 }).lean()
        ]);
        ...
        // Website Pages HTML/JSON scan for embedded image/font URLs
        websitePages.forEach(p => {
            const str = JSON.stringify(p);
            const matches = str.match(/1[0-9]{12}_[a-zA-Z0-9._-]+/g) || [];
            matches.forEach(filename => usedFilenames.add(filename));
        });

        // Website Configs
        websiteConfigs.forEach(c => {
            addUrl(c.branding?.logo);
            addUrl(c.branding?.favicon);
        });
