        ...
        CredentialsProvider({
            ...
            async authorize(credentials) {
                ...
                try {
                    ...
                    if (user && user.allowLogin === false) {
                        throw new Error("This profile is for payroll records only and does not have login access.");
                    }
                    ...
                    const userObject = {
                        ...
                        assignments: {
                            batches: (user.assignments?.batches || []).map(id => id.toString()),
                            courses: (user.assignments?.courses || []).map(id => id.toString())
                        },
    ...
    debug: process.env.NODE_ENV === 'development', callbacks: {
        async jwt({ token, user, trigger, session }) {
            ...
            if (user) {
                ...
                token.id = user.id;
                token.name = user.name || `${user.profile?.firstName || ''} ${user.profile?.lastName || ''}`.trim() || 'Student';
                ...
                token.activeSession = user.activeSession;
            ...
            if (trigger === "update" && session?.activeInstitute) {
                ...
                if (token.role === 'super_admin') {
                    ...
                    // Verify membership directly in DB
                    try {
                        await connectDB();
                        const verifiedMembership = await Membership.findOne({
                            user: token.id,
                        ...
                        console.error("=> Auth JWT Switch Error:", dbErr);
                    }
                }
            }
            return token;
        ...
        async session({ session, token }) {
            if (token) {
                ...
                session.user.name = token.name || session.user.name || `${token.profile?.firstName || ''} ${token.profile?.lastName || ''}`.trim();
                session.user.institute = token.institute;
                session.user.availableInstitutes = token.availableInstitutes;
                session.user.image = token.picture;
                session.user.profile = token.profile;
                session.user.enrollmentNumber = token.enrollmentNumber;
                session.user.activeSession = token.activeSession;
                session.user.permissions = token.permissions;
                session.user.assignments = token.assignments || { batches: [], courses: [] };
            }
            return session;
        },
    },
